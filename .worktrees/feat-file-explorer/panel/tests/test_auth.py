...
def test_check_admin_group_member():
    id_output = "uid=1001(joao.silva) gid=1001(domain users) groups=1001(domain users),10001(ti)"
    with patch("auth.run_cmd", return_value=(id_output, "")):
        assert check_admin_group("joao.silva") is True

def test_check_admin_group_non_member():
    id_output = "uid=1002(pedro.fora) gid=1001(domain users) groups=1001(domain users)"
    with patch("auth.run_cmd", return_value=(id_output, "")):
